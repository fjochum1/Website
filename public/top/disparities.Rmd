...
p_rainclound_region_IDF <-
  ...
  geom_text(aes(y=median, label=format(round(median,1), nsmall=1)),
            ...
            size=4,
            nudge_x=0.08) +
  labs(title= 'Distribution of number of first staging/debulking in <i style="color:#6ba28c;">Île-de-France</i> and outside',
       x = "", y = "") +
  theme_ipsum() +
...
map_IDF <- ggplot() +
  ...
  theme(legend.text = element_text(size = 10, hjust = 0,
        ...
        legend.title = element_text(size = 14, hjust = 0,
                                    color = "grey20",family = "Arial Narrow"),
        legend.box.background = element_rect(color="#f5f5f5", fill="#f5f5f5"),
...
```{r girafe IDF, include= TRUE, out.width='100%'}
IDF 
```

```{r p_rainclound_region_IDF_final, include= TRUE, fig.width=15, fig.height=11, out.width='100%'}
